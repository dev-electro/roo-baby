/**
 * ROO API — via SvelteKit + Cloudflare Pages adapter
 * GET  /api/health  → deployment status
 * POST /api/analyze → audio/image → Gemini analysis
 */
export async function GET({ platform }) {
	const KEY = platform?.env?.GEMINI_API_KEY || process.env.GEMINI_API_KEY;
	const MODEL = platform?.env?.GEMINI_MODEL_NAME || process.env.GEMINI_MODEL_NAME || 'gemini-2.0-flash';
	return json({
		status: 'ok',
		timestamp: new Date().toISOString(),
		config: { api_key_set: Boolean(KEY), model: MODEL }
	});
}
	...
	try {
		...
		if (a?.size > 0) {
			...
			if (!/wav|mpeg|mp3|flac|ogg|aac|m4a/i.test(mime))
				return json({ error: `Unsupported: ${mime}. Use WAV, MP3, FLAC, or OGG.` }, 400);
...
function toB64(buf) {
	...
	for (let i = 0; i < bytes.length; i += 4096) chunks.push(String.fromCharCode(...bytes.slice(i, i + 4096)));
